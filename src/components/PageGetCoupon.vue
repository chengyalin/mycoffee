...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkList (shareUid) {
        const url = '/api/coupon/couponbank/list/?share_user_id=' + shareUid + '&timestamp=' +
          this.timestamp
        this.$http.get(url).then(({data}) => {
          if (data.ok) {
      ...
      qiangCoupon (shareUid) {
        const url = '/api/coupon/couponbank/create/?share_user_id=' + shareUid +
          '&user_id=' + this.currentUser.id + '&timestamp=' +
          this.timestamp
        /*const url = 'http://www.zhongkakeji.com/api/coupon/couponbank/create/?share_user_id=29' +
